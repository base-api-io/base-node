...
describe("Images Endpoint", () => {
  ...
  describe("Getting the image url", () => {
    test("it returns the url without any params", () => {
      ...
      expect(url).toEqual("https://api.base-api.io/v1/images/image_id/version?")
    })

    test("it returns the url with quality", () => {
      const url = client.images.imageUrl("image_id", 80)

      expect(url).toEqual("https://api.base-api.io/v1/images/image_id/version?quality=80")
    })

    test("it returns the url with format", () => {
      const url = client.images.imageUrl("image_id", null, "png")

      expect(url).toEqual("https://api.base-api.io/v1/images/image_id/version?format=png")
    })

    test("it returns the url with resize", () => {
      const url = client.images.imageUrl("image_id", null, null, {width: 10, height: 8})

      expect(url).toEqual("https://api.base-api.io/v1/images/image_id/version?resize=10_8")
    })

    test("it returns the url with crop", () => {
      const url = client.images.imageUrl("image_id", null, null, null, {top: 1, left: 2, width: 3, height: 4})

      expect(url).toEqual("https://api.base-api.io/v1/images/image_id/version?crop=3_4_2_1")
    })
  })
